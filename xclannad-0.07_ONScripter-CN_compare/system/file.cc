...
int DIRFILE::CheckFileDeal(void) {
	HANDLE dir; WIN32_FIND_DATA ent;
	int flen = 0;
	
	WCHAR arcnameW[256];
	MultiByteToWideChar(CP_ACP, 0, arcname, -1, arcnameW, 256);
	wcscat(arcnameW, TEXT("/*"));
	dir = FindFirstFile(arcnameW, &ent);
	...
	do {
		char name[256];
		WideCharToMultiByte(CP_ACP,0,ent.cFileName,-1,name,256,NULL,NULL);
		count++;
		flen += strlen(name)+1;
...
void DIRFILE::ListupFiles(int fname_len) {
	HANDLE dir; WIN32_FIND_DATA ent; int i;
	fname_len *= 2;
	WCHAR arcnameW[256];
	MultiByteToWideChar(CP_ACP, 0, arcname, -1, arcnameW, 256);
	wcscat(arcnameW, TEXT("/*"));
	...
	do {
		...
		if(ent.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY){
			...
			atom.offset = 0;
			atom.arcsize = ent.nFileSizeLow;
			atom.filesize = ent.nFileSizeLow;
		}
